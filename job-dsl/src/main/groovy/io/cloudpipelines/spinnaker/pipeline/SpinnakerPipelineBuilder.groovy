package io.cloudpipelines.spinnaker.pipeline
...
import io.cloudpipelines.spinnaker.pipeline.model.Artifact
import io.cloudpipelines.spinnaker.pipeline.model.Capacity
import io.cloudpipelines.spinnaker.pipeline.model.Cluster
import io.cloudpipelines.spinnaker.pipeline.model.Manifest
import io.cloudpipelines.spinnaker.pipeline.model.Root
import io.cloudpipelines.spinnaker.pipeline.model.Stage
import io.cloudpipelines.spinnaker.pipeline.model.StageEnabled
...
class SpinnakerPipelineBuilder {
	...
	SpinnakerPipelineBuilder(PipelineDescriptor pipelineDescriptor, Repository repository,
							 io.cloudpipelines.common.PipelineDefaults defaults, Map<String, String> additionalFiles) {
	...
	private Tuple2<Integer, Stage> callJenkins(String text, String jobName, int firstRefId) {
		...
		Stage stage = new Stage(
			...
			failPipeline: true,
			job: "${io.cloudpipelines.spinnaker.SpinnakerDefaults.projectName(repository.name)}-${jobName}",
	...
	private Tuple2<Integer, Stage> runTests(String env, String text, String testName,
		...
		Stage stage = new Stage(
			continuePipeline: false,
			failPipeline: true,
			job: "${io.cloudpipelines.spinnaker.SpinnakerDefaults.projectName(repository.name)}-${env}-env-${testName}",
			master: defaults.spinnakerJenkinsMaster(),
			name: "${text}",
			parameters: [
				(EnvironmentVariables.PIPELINE_VERSION_ENV_VAR) :
	...
	protected String triggerJobName() {
		return "${io.cloudpipelines.spinnaker.SpinnakerDefaults.projectName(repository.name)}-build"
	}
}
