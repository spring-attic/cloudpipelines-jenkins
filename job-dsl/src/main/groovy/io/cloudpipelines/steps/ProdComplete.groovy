...
/**
 * Switches over the production deployment
 *
 * @author Marcin Grzejszczak
 * @since 1.0.0
 */
@CompileStatic
class ProdComplete implements Step<FreeStyleJob> {
	...
	ProdComplete(DslFactory dsl, io.cloudpipelines.common.PipelineDefaults pipelineDefaults) {
		this.dsl = dsl
	...
	CreatedJob step(String projectName, Coordinates coordinates, PipelineDescriptor descriptor) {
		...
		Job job = dsl.job("${projectName}-prod-env-complete") {
			...
			steps {
				...
				commonSteps.runStep(delegate as StepContext, "prod_complete.sh")
			}
			publishers {
