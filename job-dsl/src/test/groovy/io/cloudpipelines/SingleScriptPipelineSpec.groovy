...
class SingleScriptPipelineSpec extends Specification {
	...
	def 'should treat non-tarball and non-git URL with pipelines functions as git'() {
		...
		then:
			assertScriptForScriptsDownloading(jm, 'rm -rf .git/tools && git clone --recursive -b master --single-branch https://akjsad.com/foo/bar .git/tools')
	}
	...
	def 'should clone git repo with pipelines functions if url ends with .git'() {
		...
		then:
		assertScriptForScriptsDownloading(jm, 'rm -rf .git/tools && git clone --recursive -b baz --single-branch https://foo.com/bar.git .git/tools')
	}
	...
	def 'should clone git repo with pipelines functions with default master branch if url ends with .git'() {
		...
		then:
		assertScriptForScriptsDownloading(jm, 'rm -rf .git/tools && git clone --recursive -b master --single-branch https://foo.com/bar.git .git/tools')
	}
